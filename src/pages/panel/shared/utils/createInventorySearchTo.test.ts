...
test('createInventorySearchTo should return correct uri', () => {
  ...
  expect(
    query1.startsWith(`{"pathname":"/inventory","search":"?q=is(${kind})&changes=true&before=`) &&
      query1.endsWith('&change=node_compliant"}'),
  ).toBe(true)
  expect(
    query2.startsWith(`{"pathname":"/inventory","search":"?q=is(${kind})&changes=true&before=`) &&
