...
const PieCardItem = ({
  ...
  const showPieChartTimer = useRef<number>()
  const handleShowPieChart = () => {
    if (hidingPieChart) {
      setHidingPieChart(false)
    }
    if (showPieChartTimer.current) {
      window.clearTimeout(showPieChartTimer.current)
    }
  ...
  const handleHidePieChart = () => {
    ...
    if (showPieChart) {
      showPieChartTimer.current = window.setTimeout(() => {
        setHidingPieChart(false)
