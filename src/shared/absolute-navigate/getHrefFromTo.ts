...
export const getHrefObjFromTo = (to: To, noAutoHash?: boolean) => {
  ...
  if (typeof to === 'object') {
    const newTo = {
      ...
      hash: to.hash ?? (noAutoHash ? undefined : selectedWorkspaceId ? `#${selectedWorkspaceId}` : undefined),
      hasHash: !!to.hash,
    }
    return newTo
  }
  ...
  const hash =
    (hashSplitted[1] ? `#${hashSplitted[1]}` : undefined) ?? (noAutoHash ? '' : selectedWorkspaceId ? `#${selectedWorkspaceId}` : '')
...
export const getHrefFromTo = (to: To, noAutoHash?: boolean) => {
  const { pathname, hash, search } = getHrefObjFromTo(to, noAutoHash)
