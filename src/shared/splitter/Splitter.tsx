...
import { GutterTheme, SplitDirection } from './enums'
import './index.css'
import { isTouchDevice } from './isTouchDevice'
import { ActionType, State, reducer } from './state'
import { useEventListener } from './useEventListener'
import { flattenChildren, getGutterSizes, getInnerSize, isTouchEvent } from './utils'
...
export const Splitter = ({
  ...
  const startDragging = useCallback(
    (direction: SplitDirection, gutterIdx: number) => {
      dispatch({
      ...
      window.document.body.style.cursor = getCursorIcon(direction)
    },
  ...
  const createPairs = useCallback((direction: SplitDirection, children: HTMLElement[], gutters: HTMLElement[]) => {
    dispatch({
  ...
  const adjustSize = useCallback(
    (direction: SplitDirection, offset: number) => {
  ...
  const drag = useCallback(
    (e: MouseEvent | TouchEvent, direction: SplitDirection, minSizes: number[]) => {
  ...
  const onStopDragging = () => {
    if (!state.isDragging) return
    if (state.draggingIdx === undefined) throw new Error(`Cannot calculate sizes after dragging = 'state.draggingIdx' is undefined`)
  ...
  useEventListener('touchmove', onMove, [direction, state.isDragging, drag, minWidths, minHeights], {
  ...
  useEffect(
    function initialSetup() {
      ...
      if (childRefs.current && !childRefs.current[0].offsetParent) return
  ...
  return (
    <div className={'__dbk__container ' + `${direction}`} ref={containerRef} nonce={nonce}>
      {state.isReady &&
        children.map((c, idx) => (
          <Fragment key={idx}>
            <div
